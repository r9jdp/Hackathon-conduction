...
const GenderRadio = ({ inputState, setInputState }) => {
  const { resumeData, setResumeData } = useUserContext();
  useGSAP(() => {
    gsap.from(".genderAnimation", {
      duration: 1,
      opacity: 0,
      x: 90,
      ease: "power2.out",
  ...
  return (
    <div className=" p-10 ">
      <div className="genderAnimation flex flex-col items-start gap-4 overflow-hidden rounded-md p-6 shadow-sm shadow-[#00000050]">
      ...
      <button
        onClick={() => {
          ...
          temp.genderDetails = false;
          temp.instituteDetails = true;
          setInputState({ ...temp });
          console.log("Input component: ", temp);
        }}
        type="submit"
        className="flex m-5 buttonAnimation justify-center gap-2 items-center mx-auto shadow-xs text-lg bg-emerald-500 backdrop-blur-md lg:font-semibold isolation-auto border-gray-50 before:absolute before:w-full before:transition-all before:duration-700 before:hover:w-full before:-left-full before:hover:left-0 before:rounded-full text-gray-50 before:-z-10 before:aspect-square before:hover:scale-150 before:hover:duration-700 relative z-10 px-4 py-2 overflow-hidden border-2 rounded-full group"
      >
        Next
        <svg
